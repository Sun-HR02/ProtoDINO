...
class GroundingDINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        """添加的特征获取代码"""
        hs_tensor = torch.stack(hs, dim=0)  # 新增维度0作为层维度
        out["hs"] = hs_tensor  # 存储为张量格式
        # 修改特征提取部分
        out['target_features'] = []
        # confidence_threshold = 0.35  # 可提取为类参数
        # import ipdb;ipdb.set_trace()
        for batch_idx in range(out['pred_boxes'].shape[0]):
            # 直接获取第一一个decoder层的所有特征, 这里修改提取特征的位置
            batch_features = hs_tensor[0, batch_idx]  # 形状 [num_queries, feature_dim] 
            out['target_features'].append(batch_features)
        """ 返回添加了特征"""
